...
grep -Fq -- '--accent-fg-color: #000000;' "$css" || fail "GTK accent gets a contrast-safe foreground"
grep -Fq -- '--warning-fg-color: #000000;' "$css" || fail "GTK warning gets a contrast-safe foreground"
...
grep -Fq -- '--scrollbar-outline-color: #1a1b26;' "$css" || fail "GTK scrollbar outline follows the theme"
...
for name in \
  ...
  headerbar-bg headerbar-fg headerbar-border headerbar-backdrop headerbar-shade \
  headerbar-darker-shade sidebar-bg sidebar-fg sidebar-backdrop sidebar-border \
  sidebar-shade secondary-sidebar-bg secondary-sidebar-fg \
  secondary-sidebar-backdrop secondary-sidebar-border secondary-sidebar-shade \
  card-bg card-fg card-shade dialog-bg dialog-fg popover-bg popover-fg \
  popover-shade overview-bg overview-fg thumbnail-bg thumbnail-fg \
  active-toggle-bg active-toggle-fg shade scrollbar-outline; do
...
pass "GTK theme is installed without restarting applications"
