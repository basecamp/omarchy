...
assert(/function handleStateWatchData\(data\) \{[\s\S]*?_stateWatchBackoffMs = 2000/.test(serviceSource), 'tailscale resets the IPN watcher backoff once data flows')
...
assert(/id: connectTimeoutTimer\s+interval: 20000\s+repeat: false\s+onTriggered: \{\s+if \(root\._desired !== 1 \|\| root\.running\) return\s+root\._desired = -1/.test(serviceSource), 'tailscale drops the optimistic on state when the backend does not connect in time')
