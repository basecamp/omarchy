...
core_bin="$test_tmp/core"
test_home="$test_tmp/home"
fixture="$test_tmp/fixture"
curl_log="$test_tmp/curl"
mkdir -p "$mock_bin" "$core_bin" "$test_home/.local/bin" "$fixture/dist-package"
for command in tar gzip grep mktemp mkdir chmod mv ln rm cat dirname basename; do
  ln -s "$(command -v "$command")" "$core_bin/$command"
done
...
export PATH="$test_home/.local/bin:$mock_bin:$ROOT/bin:$core_bin"
