...
# Billing and settings probes are stubbed: the collector must not reach the network.
CACHE_HOME=$(mktemp -d)
...
user = json.loads(sys.argv[3]) if len(sys.argv) > 3 and sys.argv[3] else {}
force = True
seen = []
...
def urlopen(request, timeout=None):
  ...
  seen.append(url)
  payload = io.BytesIO()
  if "billing?format=credits" in url:
    payload.write(json.dumps(billing).encode())
  elif url.endswith("/v1/settings") or url.endswith("/settings"):
    payload.write(json.dumps(settings).encode())
  elif "user?include=subscription" in url:
    payload.write(json.dumps(user).encode())
  else:
...
billing=$(jq -nc --arg end "$period_end" '{
  config: {
    currentPeriod: {
...
limits=$(probe_record "$billing" "$settings")
[[ $(jq -r '.tierLabel' <<<"$limits") == "SuperGrok Heavy" ]] ||
  fail "Grok collector uses subscription_tier_display from settings" "$limits"
[[ $(jq -c --arg end "$period_end" '[.limits[] | {label, percent, resetsAt}]' <<<"$limits") == '[{"label":"Weekly","percent":0.24,"resetsAt":"'"$period_end"'"}]' ]] ||
...
[[ $(jq -r '._probed | length' <<<"$limits") == "2" ]] ||
  fail "Grok collector probes billing and settings once" "$limits"
pass "Grok collector reads weekly limits and extra credits"

# Without the product display string, SuperGrokPro from /v1/user is rewritten.
fallback=$(probe_record "$billing" '{}' '{"subscriptionTier":"SuperGrokPro"}')
[[ $(jq -r '.tierLabel' <<<"$fallback") == "SuperGrok Pro" ]] ||
  fail "Grok collector labels SuperGrokPro as SuperGrok Pro" "$fallback"
pass "Grok collector labels SuperGrokPro as SuperGrok Pro"

plus=$(probe_record "$billing" '{}' '{"subscriptionTier":"SuperGrokPlus"}')
[[ $(jq -r '.tierLabel' <<<"$plus") == "SuperGrok Plus" ]] ||
  fail "Grok collector labels SuperGrokPlus as SuperGrok Plus" "$plus"
pass "Grok collector labels SuperGrokPlus as SuperGrok Plus"

# A SuperGrok login with a weekly pool and no prepaid credits is a limit, not
# an empty $0.00 ledger. creditUsagePercent of 0.4 is under one percent used.
subscription=$(probe_record "$(jq -nc --arg end "$period_end" '{config:{currentPeriod:{type:"USAGE_PERIOD_TYPE_WEEKLY",end:$end},creditUsagePercent:0.4,prepaidBalance:{val:0},onDemandCap:{val:0},onDemandUsed:{val:0}}}')" "$settings")
[[ $(jq -r '.limits[0].percent' <<<"$subscription") == "0.004" ]] ||
  fail "Grok collector reads creditUsagePercent as a percentage" "$subscription"
[[ $(jq -r '.balance // "none"' <<<"$subscription") == "none" ]] ||
  fail "Grok collector omits a zero SuperGrok prepaid ledger" "$subscription"
...
# event_name plus a flat token ledger on the update object, no nested usage.
FLAT_HOME=$(mktemp -d)
trap 'rm -rf "$TEST_HOME" "$OTHER_HOME" "$CACHE_HOME" "$EXPIRED_HOME" "$UNWRITABLE_HOME" "$SHAPE_HOME" "$FLAT_HOME"' EXIT
flat_id="019fb000-0000-7000-8000-0000000000aa"
...
[[ $(jq -r '.todayPrompts' <<<"$result") == "1" ]] ||
  fail "Grok collector reads event_name turn_completed" "$result"
[[ $(jq -c '.modelUsage.grok' <<<"$result") == '{"cacheCreationInputTokens":0,"cacheReadInputTokens":10,"inputTokens":30,"outputTokens":6}' ]] ||
  fail "Grok collector reads a flat token ledger on the update" "$result"
pass "Grok collector reads event_name and a flat token ledger"

# A fork summary written after the first scan must still drop that session.
FORK_HOME=$(mktemp -d)
trap 'rm -rf "$TEST_HOME" "$OTHER_HOME" "$CACHE_HOME" "$EXPIRED_HOME" "$UNWRITABLE_HOME" "$SHAPE_HOME" "$FLAT_HOME" "$FORK_HOME"' EXIT
...
write_summary "$late_dir" "subagent" "$late_id"
result=$(HOME="$FORK_HOME" XDG_CACHE_HOME="$FORK_HOME/.cache" GROK_HOME="$FORK_HOME/.grok" \
  "$ROOT/bin/omarchy-agent-usage-grok" --force)
...
AUTH_HOME=$(mktemp -d)
trap 'rm -rf "$TEST_HOME" "$OTHER_HOME" "$CACHE_HOME" "$EXPIRED_HOME" "$UNWRITABLE_HOME" "$SHAPE_HOME" "$FLAT_HOME" "$FORK_HOME" "$AUTH_HOME"' EXIT
...
auth_pick=$(COLLECTOR="$ROOT/bin/omarchy-agent-usage-grok" HOME="$AUTH_HOME" \
  XDG_CACHE_HOME="$AUTH_HOME/.cache" GROK_HOME="$AUTH_HOME/.grok" python3 - "$billing" "$settings" <<'PY'
import importlib.machinery, importlib.util, io, json, os, sys
