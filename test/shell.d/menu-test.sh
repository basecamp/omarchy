...
const expectedAgents = {
  ...
  dim: { icon: '\ue90a', iconFont: 'omarchy', label: 'Dim Agent' },
  kimi: { icon: '\ue90b', iconFont: 'omarchy', label: 'Kimi Code' },
}
assert(
...
assertDeepEqual(
  defaultItems
    ...
    .map(item => item.label),
  ['Antigravity', 'Claude', 'Codex', 'Copilot', 'Crush', 'Dim Agent', 'Grok', 'Kimi Code', 'omp', 'OpenCode', 'Ori', 'Pi'],
