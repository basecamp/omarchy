...
acpi_migration="$ROOT/migrations/1787163407.sh"

grep -Fq 'KERNEL_CMDLINE[default]+=" intel_iommu=on iommu=pt pm_async=off mem_sleep_default=deep acpi_osi=!Darwin acpi_osi=Linux"' "$fix_t2" ||
...
acpi_repair_marker="$test_tmp/t2-acpi-repair-complete"
cat >"$limine_conf" <<'EOF'
# Generated by Omarchy installer for T2 Mac support
# Required parameters: acpi_osi=!Darwin acpi_osi=Linux
...
PATH="$stub_bin:$PATH" \
  ...
  OMARCHY_T2_LIMINE_CONF="$limine_conf" \
  OMARCHY_T2_RUNNING_CMDLINE="$running_cmdline" \
  OMARCHY_T2_ACPI_REPAIR_MARKER="$acpi_repair_marker" \
  bash -euo pipefail "$acpi_migration" >/dev/null

grep -Fq 'KERNEL_CMDLINE[default]+=" intel_iommu=on iommu=pt pm_async=off mem_sleep_default=deep acpi_osi=!Darwin acpi_osi=Linux"' "$limine_conf" ||
  fail "T2 ACPI migration adds the firmware identity parameters"
grep -Fxq 'limine-mkinitcpio' "$calls" ||
  fail "T2 ACPI migration rebuilds the boot image"
[[ -f $acpi_repair_marker ]] || fail "T2 ACPI migration records the machine-wide repair"
pass "T2 ACPI migration repairs existing installs"
...
(( $(grep -E '^KERNEL_CMDLINE\[default\]\+=' "$limine_conf" | grep -o 'acpi_osi=!Darwin' | wc -l) == 1 )) ||
  fail "T2 ACPI migration adds the Darwin override exactly once"
(( $(grep -E '^KERNEL_CMDLINE\[default\]\+=' "$limine_conf" | grep -o 'acpi_osi=Linux' | wc -l) == 1 )) ||
  fail "T2 ACPI migration adds the Linux identity exactly once"
[[ ! -s $calls ]] || fail "an already repaired T2 ACPI configuration is left unchanged" "$(cat "$calls")"
pass "T2 ACPI migration is machine-idempotent before reboot"

rm -f "$acpi_repair_marker"
: >"$calls"
...
grep -Fxq 'limine-mkinitcpio' "$calls" ||
  fail "T2 ACPI migration retries an interrupted boot image rebuild"
! grep -Fq $'sudo\tsed' "$calls" ||
  fail "T2 ACPI rebuild retry does not edit an already repaired configuration"
[[ -f $acpi_repair_marker ]] || fail "a retried T2 ACPI repair records completion"
pass "T2 ACPI migration retries an interrupted boot image rebuild"
...
! grep -q 'acpi_osi' "$limine_conf" || fail "non-T2 Limine configuration is unchanged by the ACPI migration"
[[ ! -s $calls ]] || fail "non-T2 systems skip the ACPI migration" "$(cat "$calls")"
pass "T2 ACPI migration skips unrelated hardware"
