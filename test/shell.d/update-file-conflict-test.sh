...
if grep -ERq 'omarchy-update-system-pkgs-when-conflicted|OMARCHY_UPDATE_CONFLICT|OMARCHY_UPDATE_RETRY|OMARCHY_REPLACED_DIR' "$ROOT/bin"; then
  fail "the update commands still route into automatic file-conflict recovery"
fi
if grep -q -- '--overwrite' "$update"; then
...
grep -q 'exists in filesystem' "$test_tmp/err" || fail "Pacman's filesystem-conflict report is hidden"
