...
host_fn resolve_download_file $'clip.mp4\x01--include=not-a-file' &&
  fail "yt-dlp native host rejects a path containing C0 bytes other than newline"
pass "yt-dlp native host rejects a path containing C0 bytes other than newline"

grep -Fq '[[:cntrl:]]' "$ROOT/bin/omarchy-chromium-ytdlp-host" ||
  fail "yt-dlp native host path checks reject every C0/DEL character"
pass "yt-dlp native host path checks reject every C0/DEL character"
