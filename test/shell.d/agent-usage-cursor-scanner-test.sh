...
credentials, error = mod.load_credentials(state_db, Path("/nonexistent-cursor-auth.json"))
...
print(json.dumps(mod.build_rate_limits(payload, credentials.get("membershipType")), separators=(",", ":")))
PY
)
...
print(json.dumps(mod.build_rate_limits(None, "pro")))
PY
)
...
http_ok=$(HOME="$TEST_HOME" XDG_STATE_HOME="$TEST_HOME/.local/state" python3 - "$COLLECTOR" "$state_db" <<'PY'
import importlib.util
import io
import json
import sys
...
def fake_urlopen(request, timeout=None):
  ...
  if "GetPlanInfo" in url:
    return FakeResponse(json.dumps({"planInfo": {"planName": "Pro"}}).encode("utf-8"))
  if "GetFilteredUsageEvents" in url:
    return FakeResponse(json.dumps({"totalUsageEventsCount": 0, "usageEventsDisplay": []}).encode("utf-8"))
  if "GetAggregatedUsageEvents" in url:
    return FakeResponse(json.dumps({"aggregations": []}).encode("utf-8"))
