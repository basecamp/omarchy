...
Schema-v2 QML requests an external effect with `runtime.invoke(capability, operation, arguments)`, for example `runtime.invoke("storage.private", "write", { key: "state", value: bytes })`. The worker accepts only an exact capability and operation pair allowed by that plugin revision's manifest request: built-in requests name a capability and inherit its registered operation set, while extensible requests name their operations explicitly. For an extensible capability, the worker also copies the manifest's trusted definition generation and digest into the broker envelope; the plugin cannot select or replace that authority reference. The broker still independently verifies the active grant, scope, provider, and definition before dispatch. `runtime.permissions`, `runtime.hasPermission(capability, operation)`, and `runtime.permissionState(capability, operation)` let QML hide or degrade optional features, but never authorize an effect.

Schema-v2 plugins may import `Omarchy.PluginPresentation 1.0`. This pure-QML compatibility SDK is embedded in the versioned worker and supplies authority-free controls, sizing/theme defaults, packaged-text and private-storage helpers, and a Process-like broker-call lifecycle. The SDK does not import the trusted shell's private `qs.Commons` or `qs.Ui` modules and does not expose the host Quickshell runtime, compositor objects, host processes, host files, D-Bus, shell IPC, or network access. Its broker helpers are ergonomic wrappers over the same manifest-declared `runtime` API and add no authority.

The worker also owns a deliberately narrow `Quickshell.Io 1.0` compatibility module. Its `FileView` supports read-only UTF-8 packaged assets, and its `StdioCollector` is bounded plugin-local text state. `Process` is intentionally absent until a manifest-declared callable-helper protocol can bind command identity, framing, cancellation, concurrency, and output limits without enabling ambient execution or adding process/socket syscalls after the steady-state filter.

The worker also embeds independent `Quickshell 1.0` core and `Quickshell.Widgets 1.0` compatibility subsets. They provide only render-scene composition, visual lazy loading, wall-clock state, packaged icon rendering, and single-child wrappers. `ShellRoot` maps to the worker's existing render scene rather than creating a window. Reload generation behavior, the ambient `Quickshell` singleton, native windows, compositor integrations, services, and unlisted types remain absent. Resource admission enumerates the root module files and widget files exactly; it does not admit the broader `/qt/qml/Quickshell/` tree.
