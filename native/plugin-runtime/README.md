...
Schema-v2 plugins may import `Omarchy.PluginPresentation 1.0`. This pure-QML compatibility SDK is embedded in the versioned worker and supplies authority-free controls, sizing/theme defaults, packaged-text and private-storage helpers, and a Process-like broker-call lifecycle. The SDK does not import the trusted shell's private `qs.Commons` or `qs.Ui` modules and does not expose Quickshell, compositor objects, host processes, host files, D-Bus, shell IPC, or network access. Its broker helpers are ergonomic wrappers over the same manifest-declared `runtime` API and add no authority.

Plugin-owned Qt Quick, JavaScript, assets, and pure-QML modules remain inside the worker boundary. The current certified native-QML set is deliberately smaller than unrestricted Quickshell: `QtQml`, `QtQuick`, and `QtQuick.Shapes`. Adding a Quickshell or Qt module requires an explicit resource/mount/syscall threat review and adversarial loading tests. In particular, sandbox-local execution of declared plugin helpers is distinct from exposing Quickshell's ambient `Process` API and remains deferred until the executable identity, startup timing, descendants, cancellation, and steady-state seccomp contracts are proven.
