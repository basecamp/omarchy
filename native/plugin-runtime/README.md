...
A plugin may include a bounded output name in an authenticated open or toggle surface intent. The trusted shell accepts it only when it exactly matches a currently connected host-owned screen; otherwise placement falls back to the source surface's screen and then the focused or first screen. Plugins that need an output list or window avoidance geometry request the fixed, permissioned `system.observe` datasets, which return only sanitized connector names and clipped workspace-local rectangles. A desktop-overlay manifest may opt into `initiallyVisible` so persisted plugin-owned visibility can be restored after activation without exposing shell lifecycle or compositor objects; other surface roles cannot use that field.
