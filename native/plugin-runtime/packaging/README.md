...
The packaged system observer is likewise dormant until an authorized `system.observe` call. It exposes only package-summary counts and sanitized Hyprland output/window geometry through fixed root-owned commands. The provider profile is the only profile allowed to inherit `HYPRLAND_INSTANCE_SIGNATURE` and `XDG_RUNTIME_DIR`; it does not inherit the rest of the host environment. Package names, window titles, classes, process identifiers, compositor addresses, command output, and compositor control authority remain outside the sandbox.
