...
The worker currently certifies `QtQml`, `QtQuick`, `QtQuick.Effects`, `QtQuick.Layouts`, and `QtQuick.Shapes`. Effects and Layouts are each mounted as an exact three-file native module closure and preloaded before the steady-state syscall filter; they add visual effects and geometry management but no external authority. The worker does not expose Quickshell, `Quickshell.Io`, `Quickshell.Hyprland`, `Quickshell.Wayland`, Qt Quick Controls, host D-Bus, shell IPC, host filesystem paths, or arbitrary host process execution. Selected Quickshell APIs may be added only after their native plugins, resource trees, environment dependencies, descriptors, syscalls, and behavior without host sockets have been threat-modeled and tested.
