...
CapabilityDefinition definition(){CapabilityDefinition d{.canonical_name=Name("local.status"),.authority_identity=Name("local.status-v1"),.enforcement_family=EnforcementFamily::network_fetch,.display_category_id=Name("local.services"),.display_category_label=Label("Local services"),.scope_schema=ScopeSchema::https_origins_and_methods,.title=Label("Read selected status"),.risk_text=Label("Sends a bounded request to a selected service"),.risk=RiskLevel::moderate,.revocation=RevocationPolicy::cancel_inflight,.audit={},.adapter={.adapter_class=Name("status-adapter"),.implementation_digest=digest('a'),.abi_version=1},.operations={}};d.operations.insert({.name=Name("read"),.label=Label("Read status")});d.operations.insert({.name=Name("write"),.label=Label("Change status"),.mutating=true,.requires_fresh_gesture=true});return d;}
...
void dynamic_activation_tests(){
 ...
 require(restored.binding==revision.binding&&restored.request.definition.canonical_name==revision.request.definition.canonical_name&&restored.request.definition.definition_generation==revision.request.definition.definition_generation&&restored.request.definition.definition_digest==revision.request.definition.definition_digest&&restored.request.scope==revision.request.scope&&restored.request.operations==revision.request.operations&&restored.grant.definition.canonical_name==revision.grant.definition.canonical_name&&restored.grant.definition.definition_generation==revision.grant.definition.definition_generation&&restored.grant.definition.definition_digest==revision.grant.definition.definition_digest&&restored.grant.scope==revision.grant.scope&&restored.grant.operations==revision.grant.operations&&restored.grant.epoch==8,"persisted dynamic grant did not reconstruct exactly");
 ...
 auto legacy_record=persisted;legacy_record[7]=std::byte{0};DynamicRevisionGrant rejected;require(!decode_dynamic_grant(std::span(legacy_record).first(persisted_size),rejected),"legacy dynamic grant record migrated open");
