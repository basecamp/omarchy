...
Destroying or cancelling an activation closes its channel, signals the exact direct child through a pidfd, reaps it, and prevents restart. Descendant containment and teardown require the P4 cgroup boundary and are not claimed here. The broker's existing live-generation effect lease remains the authority fence around synchronous dispatch, so activation replacement and revocation drain any bounded in-flight call before the route objects are destroyed.
