...
| `omarchy-update-system-pkgs` | Runs a plain `pacman -Syu --noconfirm` with fixed privileged binaries and no overwrite rule. Pacman file conflicts fail untouched; an attended package-vs-package conflict gets one fixed `pacman -Su` retry so the user can answer it. | **Keep.** Small leaf command with fail-closed ownership semantics. |
