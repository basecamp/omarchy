...
| `omarchy-update-system-pkgs` | Runs the root-owned package transaction helper; genuine package conflicts return to `omarchy-update-system-pkgs-when-conflicted` for an interactive retry. | **Keep for now.** Small leaf command, clear/testable. |
| `omarchy-update-file-conflicts` | Root-owned package transaction helper: captures Pacman's report, verifies file conflicts against the pending package archives, quarantines them, retries once, and safely restores or retains displaced files. | **Keep internal/hidden.** It owns the complete privilege boundary for file-conflict recovery. |
| `omarchy-update-system-pkgs-when-conflicted` | Hands a genuine package-vs-package conflict back to an interactive Pacman run (never under `-y`). | **Keep internal/hidden.** Keeps prompting out of the root file-conflict helper. |
