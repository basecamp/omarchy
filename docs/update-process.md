...
| `omarchy-update-aur-pkgs` | Reports pending AUR updates, holds them during unattended/non-terminal runs, and forces complete recipe and build-file diff review plus final confirmation during an interactive update. | **Keep during the compatibility phase.** This is an emergency review gate, not the isolated and attested pipeline described in [`aur-security.md`](aur-security.md). |
