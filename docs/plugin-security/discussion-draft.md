...
The cost is substantial. Measurements from clean commit `ec9f2a6c` count 25,624 C/C++ lines when conventional test paths and test names are excluded and 43,497 C/C++ lines in the complete reference tree. The clean Release bundle's installed `/usr` tree is 2,848,726 bytes. Its host is 1,324,296 bytes, private worker 596,824 bytes, permission CLI 456,432 bytes, and audit CLI 330,624 bytes. A full `BUILD_TESTING=ON`, two-job Release build on the development machine completed in roughly 67 seconds; that is a local observation, not a CI target. The verified bundle digest is `36b06e268e6d8bd51c8f64cfff34ca52a08790bb0cb6c3b710e3034a9f94000c`.
