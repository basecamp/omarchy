...
**System tools** include aligners like bowtie2, bwa, STAR, and HISAT2, format converters like samtools and bcftools, and utilities for genomic intervals such as bedtools. Quality control tools like fastqc and trimmomatic are included, along with multiple sequence alignment programs like mafft and muscle. RNA-seq quantification tools salmon and kallisto provide fast transcript abundance estimation. Variant calling is supported through GATK and freebayes. Sequence similarity searches use BLAST+, while genome assembly uses SPAdes. Phylogenetic analysis includes RAxML and other tools. Visualization tools like igv and pymol round out the system packages.

**Python packages** build on Omarchy's Python development environment. The scientific computing stack includes numpy, scipy, pandas, matplotlib, and seaborn. Biology-specific libraries like biopython, biotite, pysam, and scikit-bio handle common file formats and algorithms. Genomic data structures are supported through pyranges, with database access via pyensembl and pybiomart. For single-cell analysis, scanpy, anndata, muon, and squidpy are included. Differential expression analysis uses pydeseq2, while pathway analysis uses gseapy. Drug discovery applications can use deepchem. Hi-C data analysis uses cooler. Interactive work is supported through jupyter and jupyterlab, while napari provides image analysis capabilities.

**R packages** focus on Bioconductor and the tidyverse. Core infrastructure packages include Biostrings, GenomicRanges, GenomicFeatures, BSgenome, and SummarizedExperiment for handling genomic data structures. Variant annotation uses VariantAnnotation. Differential expression analysis is supported through DESeq2, edgeR, and limma, with downstream analysis via fishpond. Batch correction uses sva. Pathway and enrichment analysis uses fgsea and clusterProfiler. Single-cell analysis uses Seurat, SingleCellExperiment, scater, and monocle3 for trajectory analysis. ChIP-seq peak annotation uses ChIPseeker. The phyloseq package handles microbiome data. Visualization includes ComplexHeatmap for advanced heatmaps. The full tidyverse is included for data manipulation and visualization.
