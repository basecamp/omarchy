...
`/etc/os-release` is the standard generated selector exception: the relative symlink itself is runtime filesystem state, while the Omarchy data it selects is package-owned under `/usr/share/omarchy/etc-overrides/`. Removing the settings package restores the selector to `../usr/lib/os-release`.
...
| Package-owned system path | Add the source under `default/` or `etc/` and its PKGBUILD install line in the same change; never seed the path unowned first or rely on the updater to overwrite it |
