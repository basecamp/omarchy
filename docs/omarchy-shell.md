...
Every icon in the bar is sized by the pixels it paints, not by its asset dimensions or font metrics. `shell/Ui/WidgetButton.qml`, the base of every bar button first- and third-party, treats a lone icon glyph, a glyph beside a label (`"󰄀 4"`, `"21% 󰁹"`) and any `iconComponent` the same way: the icon's ink is measured (`shell/Ui/InkMeasure.qml` renders the item and has ImageMagick reduce it to the shape it paints, blur that into one soft mass and cut the mass at half its peak, straight and turned 45° for the diagonals) and the icon is scaled to fill `[bar] icon-canvas` and shifted until its weight sits on the canvas center.

Size and position answer two different questions. **Size** comes from how far the ink reaches, read through the blur so that a hairline or a stray serif — whose blurred mass never reaches half the peak — stops deciding how big the whole icon is. **Position** comes from where the ink weighs. Lining up the box around the ink instead leaves a mark whose weight sits high in that box (an arc) riding high on the bar, and one whose weight sits low (a body under a thin antenna) sagging, even when every box is perfectly centered; lining up the weight is what makes a row read as level. An icon is only moved as far as its box has room, plus the pixel of play the rules already allow at the canvas edge.

The canvas itself is measured from the font rather than set as a bare number: `[bar] icon-canvas` defaults to the height the icon block is drawn to at `[bar] icon-font`, so fitting an uneven row evens it without enlarging it — icons that already filled that box keep the size they always had, and only the ones drawn small grow to join them. A theme that pins `icon-canvas` gets exactly what it asks for.

That canvas is one square of a grid, and a mark may take more than one of them along the bar (across a horizontal bar, down a vertical one). An icon is fitted inside its canvas by whichever axis binds first, so a mark twice as wide as it is tall would otherwise be shrunk until its width fits one square and end up reading half the height of everything beside it. Giving it two squares lets it stay full height instead. `IconRules.squares()` rounds the mark's aspect to the nearest whole number rather than up — rounding up would hand a second square to a 1.1:1 icon and most of a row would be two squares wide, which is no grid at all — and grants the extra squares only when the mark is *cleanly* that many squares wide (within `IconRules.squareFit`). An awkward in-between shape, half again as wide as it is tall, would sit in a wider slot under a wider open-panel mark and still be no taller than before, so it stays in one square exactly as it was; so does anything past `IconRules.maxSquares`. The slot grows by whatever the extra squares add, so the padding around a two-square icon matches the padding around a one-square one.

The aspect comes from the same measured ink the rules are judged on, not from the font's metrics, which disagree with what actually rasterizes. Because a render is grabbed at the size of the canvas, ink hanging outside it is cut off by the grab, so the aspect keeps being taken until the fit settles and is then frozen — otherwise a mark near the rounding line would swap between one square and two forever, each canvas making the other look right.

 Glyphs are re-rasterized at a fractional font size and re-measured until they stop improving — a native glyph can only be placed on whole device pixels, so the last fraction of a pixel is not correctable and a pass that no longer improves has found that floor — components are transformed, and raster images go through `shell/Ui/AutoCropImage.qml`, which the tray uses. Text keeps its type metrics; a widget whose glyph is a text-sized marker in a run of text sets `normalizeIcon: false` (the workspace dot). Icon-only buttons take `[bar] icon-slot` so they space like built-in icons, and `BarIndicator` declares its smaller canvas.

The rules live in one place, `shell/Commons/IconRules.qml`. Within a tolerance of one logical pixel — or of one measured pixel, when the render is coarser than that — an icon must `fill` its canvas along one axis, be `balanced` on it (its weight on the canvas center) and stay `contained` by it, corners included. Every button verifies its final render against them and exposes `inkCompass` and `inkViolations`; `omarchy-dev-bar-icon-audit` collects these from the live bar through `omarchy-shell shell auditIcons`, prints each icon's margins alongside `BX`/`BY` — how far its weight sits off center — and fails when any shown icon breaks a rule.

A mark's shape is everything it paints above antialiasing fringe, taken **before** anything is measured, so how brightly a part was painted never moves the icon: a logo drawn in two tones measures exactly like the same logo drawn in one. Brightness is then a rule of its own — one mark, one brightness, unless the mark really is drawn in two tones. `WidgetButton` weighs the faded parts of an icon against the whole of it: a second tone covering at least `IconRules.twoToneMinShare` of the mark is a two-tone logo and is left as its author drew it, while anything less is one part faded against the rest of its own icon by accident and goes back to full. A fade covering every part alike never registers, which is right — that is a widget signalling a state over its icon, not the icon painted in two brightnesses. Set `flattenIcon: false` to opt out entirely.
