...
You will set the account's login password during setup. Omarchy disables automatic administrator login when the first managed account is added, so after logging out or rebooting you can choose between the administrator and managed accounts at the login screen.
