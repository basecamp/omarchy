...
Backgrounds can be videos as well as stills. Drop an `mp4`, `m4v`, `mov`, `webm`, `mkv`, or `avi` file in the same folder and it appears alongside the images, playing muted and looped. Playback stops on its own whenever nothing can see it — while a fullscreen window is focused, while the screen is locked, and while the screensaver is up — but a video wallpaper still costs far more power than a still one, and each monitor decodes its own copy.
