...
The Toggle menu also carries a few things that aren't `omarchy toggle` commands but behave the same: battery percentage in the bar, workspace layout (`Super + L`), window gaps (`Super + Shift + Backspace`), and the 1-window half tile (`Super + Ctrl + Backspace`). With that last one on, a lone tiled window sits on the left or right half of the screen; `Super + Shift + Left` / `Right` parks it on that half, and still swaps when there is a second window.
