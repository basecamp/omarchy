...
The prebuilt Voxtype binary requires a CPU with the x86-64-v3 instruction baseline (AVX2, FMA, BMI1/2, F16C, MOVBE — Haswell-era or newer, roughly 2013+). On older CPUs, the installer offers to build Voxtype from source via the AUR instead; this takes 20+ minutes and may run noticeably slower.
