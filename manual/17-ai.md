...
If your machine carries a big GPU, _Install > AI > Local AI Server_ turns it into its own inference box. There are two recipes: **fast** (Qwen3.8 27B) runs on a single NVIDIA card with 24 GB of VRAM, and **smart** (Qwen3.6 35B) needs two. Setup counts your qualifying GPUs, picks the smartest recipe that fits, and scales it to the hardware — more cards mean more tensor parallelism and a larger context window, but the choice you make is always just fast or smart; the quantization and serving engine underneath are the recipe's own business. The model is served in a Docker container bound to localhost, and the pi and omp agents gain a `local` provider pointing at it, so the default agent can run entirely on local compute. If you haven't picked a default agent yet, `omarchy default agent pi` completes the loop.

Once set up, a robot icon appears in the top bar. Left-click opens the Local AI panel: the serving state, a load/unload button, and the two modes — click the one you want and it serves in a floating terminal. Right-click the icon to load or unload directly (the weights stay cached, so loading is quick and unloading frees the VRAM for games or training). The same lifecycle is available as `omarchy ai start`, `omarchy ai stop`, `omarchy ai status`, and `omarchy ai list`, with `omarchy ai logs` tailing the server. _Remove > AI > Local AI Server_ (or `omarchy ai remove`) takes it all out again, downloaded weights included.

The recipe catalog isn't fixed: `omarchy ai sync` scans the GitHub accounts listed in the catalog for repos carrying an `omarchy-recipe.json` and merges what it finds — publishing an updated `fast` or `smart` for every Omarchy machine you own is just pushing that one file to a repo.
