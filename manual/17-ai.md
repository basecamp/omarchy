...
Recipes come from the [local-ai-registry](https://github.com/0xSero/local-ai-registry). Every launchable recipe is `validated`, pins its image by digest, and pins its model revision by commit. See the README under `$OMARCHY_PATH/shell/plugins/panels/local-ai/` for the state machine and safety gates.
