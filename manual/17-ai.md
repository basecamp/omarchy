...
Machines with a vendor GPU and Docker can also run pinned registry recipes from the Local AI bar widget. Add it with `omarchy bar put omarchy.local-ai --before omarchy.agents`. The compact popup scans hardware and opens Pi; the full panel lists matching recipes so you can download, run, switch, or unload one model at a time. The OpenAI-compatible endpoint binds to `127.0.0.1`, and only containers this plugin labeled are ever touched.
...
omarchy local ai open-agent pi
omarchy local ai default
omarchy local ai unload
omarchy local ai remove <recipe>
```

Recipes come from the [local-ai-registry](https://github.com/0xSero/local-ai-registry), checked out at a revision pinned with each Omarchy release. Every launchable recipe is `validated`, pins its image by digest, and pins its model revision by commit. The controller runs one labeled container at a time, binds its API to `127.0.0.1`, and refuses any recipe that fails those checks or mounts a path outside its allowlist — a refused recipe stays visible in the panel with the reason.
