...
If your machine carries a big GPU, _Install > AI > Local AI Server_ turns it into its own inference box. The validated **fast** recipe (Qwen3.8 27B) runs on a single NVIDIA card with 24 GB of VRAM and scales across additional qualifying cards. The catalog can also show candidate recipes such as **smart** (Qwen3.6 35B), but candidates stay disabled until they pass Omarchy's completion acceptance gate. The quantization and serving engine underneath are recipe internals. The model is served in a Docker container bound to localhost, and the pi and omp agents gain a `local` provider pointing at it, so the default agent can run entirely on local compute. If you haven't picked a default agent yet, `omarchy default agent pi` completes the loop.

Setup needs Docker, the NVIDIA container runtime, and at least 20 GB of free disk. Run `omarchy ai doctor` for exact readiness checks, then `omarchy ai setup` to select the validated recipe automatically (or pass a recipe name explicitly).

Once set up, a robot icon appears in the top bar. Left-click opens the Local AI panel: the serving state, a load/unload button, and the available catalog modes. Right-click the icon to load or unload directly (the weights stay cached, so loading is quick and unloading frees the VRAM for games or training). The same lifecycle is available as `omarchy ai start`, `omarchy ai stop`, `omarchy ai status`, and `omarchy ai list`, with `omarchy ai logs` showing the server logs. _Remove > AI > Local AI Server_ (or `omarchy ai remove`) removes the container, Omarchy's agent wiring, and active state while keeping cached images and weights for reuse.

The recipe catalog isn't fixed: `omarchy ai sync` refreshes recipes for your hardware from the trusted `0xSero/inference-index` GitHub registry. Omarchy follows the registry's exact recipe leaves and accepts validated status only when the catalog and leaf agree that the recipe is validated and launchable; unsupported launch contracts stay out of the local cache rather than being guessed at.
