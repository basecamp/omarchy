...
`omarchy localai` detects your GPU, asks the read-only Inference Index API for recipes validated on that hardware, and shows the balanced recommendations. The registry response pins the model revision, container digest, engine configuration, measured limits, and acceptance evidence.

```bash
omarchy localai
omarchy localai list fast
omarchy localai run --dry-run
omarchy localai run
omarchy localai status
omarchy localai logs --follow
omarchy localai stop
```

The default launch binds the OpenAI-compatible endpoint to `127.0.0.1:8000`; it is not exposed to the LAN or Tailnet. `run --dry-run` prints the exact Docker command without pulling an image, creating directories, or starting a container. Candidate recipes never appear in the launch API.

LM Studio and Ollama remain available under _Install > AI_ for people who want their own model browser or a general-purpose local model manager.
