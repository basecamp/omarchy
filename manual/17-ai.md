...
| Command    | Agent                                                            |
|------------|------------------------------------------------------------------|
| `afk`      | [AFK](https://afk.mooglest.com) (browser-first; opens the hosted app, not a TUI). Installing AFK also adds the [AFK Monitor](https://git.mooglest.com/mooglest/omarchy-afk-monitor) bar plugin. |
...
To wrap an additional CLI the same way, run `omarchy-mise-install <package> [command-name]`. The stubs are kept current along with everything else mise manages when you run `omarchy update` (or the `mup` alias). Installing or first-running `afk` also clones the AFK Monitor bar plugin (`lcavadas.afk-monitor`) if it is not already present.
...
Once you've chosen, `Super + Shift + Ctrl + A` launches the default agent in a dedicated terminal window (or brings up the picker if you haven't chosen yet). AFK is the exception: it opens the authenticated New Session flow in the browser instead of a TUI, with the current directory supplied as the project. A prompt launch (`omarchy agent prompt "Review this project"`) also prefills the task; AFK resolves the workspace or organisation, daemon, model, permissions, and isolation in the browser before you confirm the session. Agents launched this way run unattended in their respective don't-stop-to-ask modes, so be ready for them to actually do things! And since agents refuse to remember trust for your home directory, launches from `$HOME` start in `~/Work` instead.
