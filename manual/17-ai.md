...
The recipe catalog isn't fixed: `omarchy ai sync` scans the GitHub accounts listed in the catalog for repos carrying an `omarchy-recipe.json`. Synced recipes always enter as disabled candidates; discovery alone cannot mark code as validated or replace a validated shipped recipe.
