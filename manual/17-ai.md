...
The diagnosis runs in a dedicated terminal. A full investigation takes a few minutes, and the window says so before Hermes goes quiet — the answer appears when it's done. The window then stays open: type a follow-up question to keep asking about the same crash in the same conversation, or press Enter to close.
