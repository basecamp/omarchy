...
Machines with a vendor GPU and Docker can also run the recommended local model as one button on the bar. Add the widget with `omarchy bar put omarchy.local-ai --before omarchy.agents`. The popup shows the model picked for your detected hardware (plus up to four alternatives): one click downloads and loads it — the size is shown before anything lands on disk — one click unloads it, one click opens your default Omarchy agent on the running model, and one click shares the endpoint on your tailnet. The OpenAI-compatible endpoint binds to `127.0.0.1` unless you share it, and only containers this plugin labeled are ever touched.

The same lifecycle is available from the terminal:

```bash
omarchy local ai load <recipe>
omarchy local ai unload
omarchy local ai open-agent
omarchy local ai share
omarchy local ai default
omarchy local ai remove <recipe>
```

Recipes come from the [local-ai-registry](https://github.com/0xSero/local-ai-registry), checked out at a revision pinned with each Omarchy release. Every launchable recipe is `validated`, pins its image by digest, and pins its model revision by commit. The controller runs one labeled container at a time and refuses any recipe that fails those checks or mounts a path outside its allowlist — a refused recipe is reported with its reason.
