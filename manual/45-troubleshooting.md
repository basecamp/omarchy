...
This is a known upstream AMD xHCI resume bug — the controller fails to redeliver its MSI/MSI-X interrupts after waking ([Bugzilla 221073](https://bugzilla.kernel.org/show_bug.cgi?id=221073)) — not something caused by Omarchy. The reliable workaround is the lighter `s2idle` sleep state, where the USB controllers stay powered and the broken resume path is never taken: add `mem_sleep_default=s2idle` to your kernel command line in the boot loader (Limine by default). It uses a bit more battery while suspended. Note that disabling USB autosuspend alone does not help: Omarchy's shipped `/etc/modprobe.d/omarchy-usb-autosuspend.conf` is inert for this, because `usbcore` is built into the kernel and modprobe.d options never apply to built-in modules — the fix has to come from the kernel command line (or a udev rule that reinitializes the controller on resume).
