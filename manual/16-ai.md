...
Omarchy treats AI coding agents as first-class citizens, but it doesn't pick a favorite for you. Most major coding-agent CLIs come pre-wired as tiny [mise](https://mise.jdx.dev/)-managed lazy stubs in `~/.local/bin/`, so nothing is downloaded until the first time you actually run one. Cursor Agent uses Cursor's official installer and self-updater instead. Invoke any installed agent and authenticate when prompted:
...
| `agent`    | [Cursor Agent](https://cursor.com/docs/cli/overview)             |
...
To wrap an additional mise-supported CLI the same way, run `omarchy-mise-install <package> [command-name]`. The stubs are kept current along with everything else mise manages when you run `omarchy update` (or the `mup` alias). Cursor Agent updates itself automatically; you can also update it manually with `agent update`.

### The default agent

Pick your default agent with `omarchy default agent <name>` or under _Setup > Defaults > Agent_ in the Omarchy Menu (`Super + Space`). If the agent isn't installed yet, picking it installs it first. Selecting Cursor there runs its [official installer](https://cursor.com/docs/cli/installation) on demand. A fresh Omarchy will invite you to make this choice with a one-time notification.
