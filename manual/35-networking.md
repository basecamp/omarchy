...
Every list in the panel folds away by clicking its heading, and the heading keeps its summary while folded — the active profile, how many routes are selected, how many peers are connected. Peers start folded, because a real network is dozens of rows and the count is usually the whole answer; open it once and it stays open. Each peer row shows how much has flowed over it and when it was last reachable, so an idle peer reads as idle rather than broken. Below the peers, a health block covers the three pieces that fail independently — the management server, the signal server, and relay availability — plus whether your tunnel is kernel or userspace WireGuard. Above them the SSO session counts down, turning urgent in its last hour, because the tunnel keeps reporting itself healthy right up to the moment that session lapses.
