...
- 15-inch models (`MacBookPro13,3` / `14,3`) cannot resume from suspend or s2idle. The installer locks the lid instead of sleeping, puts the Radeon on the POWER_SAVING profile, and overlays power-saver (screen backlight off) while the lid is shut so the chassis does not cook. Opening the lid restores the remembered AC/battery power profile. The discrete GPU stays on.
