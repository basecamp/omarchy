...
local user_opts = {
  ...
  windowcontrols_min_hover = "#1d3b30", -- color of minimize window controls on hover
...
local function get_locale_from_json(path)
  ...
  if not json_table then
    msg.error("JSON parse error in " .. expand_path .. ": " .. tostring(parse_error))
  end
  return json_table
end
...
local function validate_user_opts()
  ...
  if user_opts.volume_control_type ~= "linear" and user_opts.volume_control_type ~= "logarithmic" then
    msg.warn("volume_control_type cannot be '" .. user_opts.volume_control_type .. "'. Setting it to 'linear'.")
