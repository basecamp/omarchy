...
sudo pacman -S --noconfirm --needed base-devel git autoconf automake libtool make \
  pkgconf glib2 glib2-devel libmspack rpcsvc-proto fuse3 procps-ng xmlsec gtkmm3 xclip clipnotify
...
# Create Wayland-to-X11 clipboard bridge for guest-to-host sync
echo "Creating clipboard bridge for guest-to-host clipboard sync..."
sudo tee /etc/systemd/user/vmware-clipboard-bridge.service <<'EOF' >/dev/null
[Unit]
Description=VMware Wayland Clipboard Bridge (guest-to-host sync)
After=graphical-session.target
PartOf=graphical-session.target

[Service]
Type=simple
ExecStart=/bin/sh -c 'sleep 3 && while true; do wl-paste -n --watch xclip -selection clipboard -r -i 2>/dev/null || sleep 1; done'
Restart=always
RestartSec=3
...
sudo systemctl --global enable vmware-clipboard-bridge.service

# Create X11-to-Wayland clipboard bridge for host-to-guest sync
echo "Creating reverse clipboard bridge for host-to-guest clipboard sync..."
sudo tee /etc/systemd/user/vmware-clipboard-bridge-x11.service <<'EOF' >/dev/null
[Unit]
Description=VMware X11 to Wayland Clipboard Bridge (host-to-guest sync)
After=graphical-session.target
PartOf=graphical-session.target

[Service]
Type=simple
ExecStart=/bin/sh -c 'sleep 3 && last=""; while clipnotify -s clipboard; do current=$(xclip -selection clipboard -o 2>/dev/null | md5sum); if [ "$current" != "$last" ] && [ -n "$current" ]; then xclip -selection clipboard -o 2>/dev/null | wl-copy -n 2>/dev/null; last="$current"; fi; done'
Restart=always
RestartSec=3
...
sudo systemctl --global enable vmware-clipboard-bridge-x11.service
