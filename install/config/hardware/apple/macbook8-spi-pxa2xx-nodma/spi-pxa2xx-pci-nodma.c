 ...
 *
 * MacBook8,1 (Wildcat Point, PCI 8086:9ce6) quirk:
 *   The DesignWare DMA controller's hardware handshake signal from the GSPI
 *   never asserts on these machines, so DMA transfers stall indefinitely.
 *   This driver overrides the lpss setup for LPT1_1 to force PIO mode.
 *   The kernel cmdline must include "irqpoll" so the SPI IRQ handler is
 *   polled even when the IO-APIC does not deliver IRQ 21.
 */
...
/*
 * MacBook8,1 quirk: Wildcat Point GSPI (8086:9ce6)
 *
 * The DesignWare DMA hardware handshake signal from the GSPI never asserts on
 * MacBook8,1 hardware. IRQ 20 (dw_dmac) and IRQ 21 (GSPI) both remain at
 * zero count across all tested kernels; DMA transfers stall indefinitely and
 * time out with -ETIMEDOUT. Forcing PIO mode (enable_dma=0) and booting with
 * irqpoll so the SPI IRQ handler is polled unconditionally restores full
 * keyboard and trackpad function.
 */
static int lpss_lpt1_1_spi_setup(struct pci_dev *dev, struct pxa2xx_spi_controller *c)
{
	int ret = lpss_spi_setup(dev, c);
	if (ret)
		return ret;
	c->enable_dma = 0;
	return 0;
...
static const struct pxa_spi_info lpss_lpt1_1_info_config = {
	.setup = lpss_lpt1_1_spi_setup,
};
...
static const struct pci_device_id pxa2xx_spi_pci_devices[] = {
	...
	{ PCI_DEVICE_DATA(INTEL, LPT1_1, &lpss_lpt1_1_info_config) },
	{ }
};
