...
| `cursor` | Cursor `state.vscdb` or Agent CLI `auth.json` + `GetCurrentPeriodUsage` | `GetFilteredUsageEvents` (day / prompts / sessions) and `GetAggregatedUsageEvents` (model totals); tier from `GetPlanInfo` |
...
IDE or Agent CLI so the collector can read a local session token
(`CURSOR_STATE_DB` / `CURSOR_AUTH_JSON` override the paths). Day and model
charts come from Cursor's usage dashboard APIs, not local transcript files.
Fireworks reads
