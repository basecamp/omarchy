...
| `opencode-go` | OpenCode's official usage endpoint (rolling 5-hour, weekly, monthly) | opencode's own database, messages on the `opencode-go` provider |
...
signed in there. OpenCode Go reads `OPENCODE_GO_API_KEY` first, then the
