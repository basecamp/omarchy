...
| `grok` | SuperGrok weekly (or monthly) pool from the cli-chat-proxy billing endpoint, plus prepaid / leftover on-demand credits as a balance when the ledger is non-zero | `$GROK_HOME/sessions` `turn_completed` events (default `~/.grok`) |
...
signed in there. Grok reads the OIDC token already stored in `~/.grok/auth.json`;
it never writes credentials. Run `grok login` to refresh an expired sign-in.
