...
| `grok` | SuperGrok weekly (or monthly) credit pool from Grok's cli-chat-proxy billing endpoint; prepaid / leftover on-demand credits as a balance | `$GROK_HOME/sessions` (default `~/.grok/sessions`) `turn_completed` usage; subagent forks are skipped so parent totals are not counted twice |

Claude limits need a signed-in CLI; without credentials the panel says so and
falls back to local stats only. A non-default Claude directory is honored via
`CLAUDE_CONFIG_DIR`, Codex via `CODEX_HOME`, Grok via `GROK_HOME`. Fireworks
reads `FIREWORKS_API_KEY` and `FIREWORKS_ACCOUNT_ID` first, then
`~/.fireworks/auth.ini` (which `firectl set-api-key` creates), then the key
opencode stores in `~/.local/share/opencode/auth.json` when Fireworks is
signed in there. Grok reads `~/.grok/auth.json` (or `$GROK_HOME/auth.json`)
and never writes the token into the usage record. A SuperGrok login with a
weekly pool and no prepaid credits is reported as a limit, not an empty
balance.
