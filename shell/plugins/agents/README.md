...
| `grok` | SuperGrok weekly pool and extra credits from Grok Build's cli-chat-proxy billing/user endpoints | `$GROK_HOME/sessions` (default `~/.grok/sessions`) `turn_completed` usage; `subagent` and `subagent_fork` sessions are skipped so parent totals are not counted twice |
