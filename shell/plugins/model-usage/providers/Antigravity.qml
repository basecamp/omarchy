...
Item {
  ...
  property string authHelpText: "Open the Antigravity IDE to load usage limits."
  property bool hasLocalStats: true

  property var providerSettings: ({})

  readonly property string scannerPath: String(Qt.resolvedUrl("../scripts/antigravity_usage_scanner.py")).replace("file://", "")
  ...
  Process {
    ...
    stderr: StdioCollector {
      onStreamFinished: if (text.trim() !== "") console.warn("model-usage/antigravity", text.trim())
    }
  }
  ...
  function parseScannerOutput(output) {
    ...
    try {
      ...
      root.authHelpText = data.authHelpText || "Open the Antigravity IDE to load usage limits."
    } catch (e) {
      console.error("model-usage/antigravity", "Failed to parse scanner output:", e, raw)
      root.usageStatusText = "Antigravity scan failed"
