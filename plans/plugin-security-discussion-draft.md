...
This branch includes the current protected `quattro` history and is a reference implementation and security proof, not a production-ready replacement for today's plugin path. The native contracts, sandbox, authenticated channels, offscreen QML worker, frame transport, broker, grants, lifecycle, recovery, representative fixtures, migration reports, and adversarial tests exist and have passed their focused gates. The final-tip package installs one `Omarchy.PluginHost` QML module containing both `PluginHostInfo` and `RemotePluginSurface`, but the host remains deliberately unavailable and feature-gated; the production executable does not yet compose the full lifecycle/broker/render path, the reference CLIs have no end-user router entries, and the user service is neither enabled nor started. The package recipe is not committed in the package repository, and a clean fresh-ISO VM run has not happened.
...
| Packaging proof | A clean detached clone at `b28f655f74ba35651797c143bdd907013fab13a7` produced `omarchy-dev-4.0.0.r1953.gb28f655-1-x86_64.pkg.tar.zst` with SHA-256 `766166c6ba959a461eed199d12a62b4ce4b1f6c36bec11c8cb41023ba445fe77`; its Release package check passed 54/54 aggregate CTests, and both the archive verifier and its negative mutation suite passed. The package has no install script or enablement symlink, and focused shell tests confirm no first-run, migration, or end-user activation path |
