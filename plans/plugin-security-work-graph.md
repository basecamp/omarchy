...
| `B0` | complete | `wave0_native_build` | Commit `96ebb07f`; inert host/worker/bridge skeleton, staged install, QML import, direct-worker denial, and focused tests pass. Final PR audit leaves the reference unit disabled and removes migration/first-run/end-user command activation until package ownership and product rollout land together |
...
| `D1` | complete | `wave0_channel` | Commits `a97885d7` and `f2b27ef7`; exact launch identity and live-generation binding, three-role negotiation/readiness, authenticated broker ingress, descriptor cleanup, sticky bounded teardown, silent-exit detection, and real-Bubblewrap proofs |
...
| `D3` | complete | `wave0_trust_map` | Commit `a7d5466a`; named manifest surfaces bind to exact activation identity, host geometry/DPR, monotonic pacing, bounded input regions and capture, gesture/focus and lock gates, fail-closed teardown, and host-owned inspection actions |
...
| `F1` | complete | `wave0_channel` | Commits `c773eeda` and `a413699c`; fixed-window request accounting, clock-regression denial, 10,000 malformed envelopes, role/correlation/frame limits, real Bubblewrap paths, stress, and sanitized defensive campaigns pass |
...
| `F5` | complete | primary, `wave0_trust_map` | Final candidate `0dac331a` passes exact clean-clone Debug/Release 55-test and package-verifier/negative suites. Hosted run `33221534246` built ISO/package from pinned inputs and passed all seven exact dormant/reference VM assertions, including installed identity, direct-worker denial, disabled/inactive service, graphical bridge presence, and feature-gated QML state. The aggregate VM suite retained five known non-plugin baseline failures; production activation remains out of scope |
...
| `G4` | complete for reference review | `wave0_channel`, `wave0_native_build` | Final candidate `0dac331a` passes fresh outside-confinement Debug and Release aggregates at 55/55, repeated fake/real channel and teardown stress, exact package verification, and the pinned dormant/reference fresh-VM gate. The host remains inert, all automatic/user-facing activation is absent, the package recipe is not landed, and the VM aggregate retains five known non-plugin baseline failures; G4 authorizes architecture/reference review only, not production activation or merge readiness |
