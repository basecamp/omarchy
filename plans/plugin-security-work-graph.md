...
| `F5` | complete | primary, `wave0_trust_map` | Final candidate `0dac331a` passes exact clean-clone Debug/Release 55-test and package-verifier/negative suites. Hosted run `33221534246` built ISO/package from pinned inputs and passed the separate exact installed-identity assertion plus all seven dormant/reference plugin-runtime VM assertions. The aggregate VM suite retained five known non-plugin baseline failures; production activation remains out of scope |
