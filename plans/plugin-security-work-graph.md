...
| `G4` | active | `wave0_channel`, `wave0_native_build` | Commits `c19174a7` and `b2614bb2`; production targets and the single installed QML module are aggregate-integrated, the lifecycle/E5 uniform sanitizer set passes at the default 8 MiB stack, and the real-Bubblewrap post-reap pidfd teardown defect found by independent review is fixed with deterministic and repeated evidence; the full outside-confinement Release aggregate passes 54/54. The final production-code scan closed bounded manifest special-file/growth reads and unknown wire/surface enum defaults with focused Debug, Release, and sanitizer evidence. Final PR audit removed all automatic/user-facing activation, while a matching final-tip package and F5 clean-install/VM evidence remain pending |
