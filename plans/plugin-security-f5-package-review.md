...
```bash
native/plugin-runtime/packaging/verify-package.sh /tmp/omarchy-pkgs-f5-final-27cea38d/pkgbuilds/omarchy-dev/omarchy-dev-4.0.0.r1951.g27cea38-1-x86_64.pkg.tar.zst
native/plugin-runtime/packaging/verify-package-test.sh /tmp/omarchy-pkgs-f5-final-27cea38d/pkgbuilds/omarchy-dev/omarchy-dev-4.0.0.r1951.g27cea38-1-x86_64.pkg.tar.zst
```

The archive and digest above supersede the historical `c19174a7`, `7aaa9b48`, and intermediate `04f8776b` package-shape runs. It includes the final dormant-integration correction: the unit is conditional on the optional host binary, the package has no install script or enablement symlink, and neither first-run nor migration code enables or starts it. `pacman -Qip` reports `Install Script : No`; the focused systemd and plugin-runtime shell tests pass the dormant/no-end-user-route assertions. This proves the archive installs the unit file without activating it; a live user-manager inactive-state observation remains part of the fresh-VM gate.
